...
{
    // モデルと同じ名前のファイルから読み込む
    const string directoryPash = baseDirectory + modelName + "/";
    // 拡張子,fbxを付加
    const string fileName = modelName + ".fbx";
    // 連結してフルパスを得る
    const string fullpash = directoryPash + fileName;

    // ファイル名を指定してFBXファイルを読み込む
    if (!fbxImporter->Initialize(fullpash.c_str(), -1, fbxManager->GetIOSettings())) {
        assert(0);
    }
...
void FbxLoader::ParseNodeRecursive(Model* model, FbxNode* fbxNode, Node* parent)
{
    ...
    node.transform *= matScaling; // ワールド行列にスクリーンを反映
    node.transform *= matRotation; // ...回転を反映
    node.transform *= matTranslation; // ...平行移動を反映
...
{
    //スキニング情報
    FbxSkin* fbxSkin = static_cast<FbxSkin*>
        (fbxMesh->GetDeformer(0, FbxDeformer::eSkin));
    //スキニング情報がなければ終了
    if (fbxSkin == nullptr)
    {
        //各頂点について処理
    ...
    for (int i = 0; i < clusterCount; i++)
    {
        ...
        //初期姿勢行列の逆行列を得る
        bone.invInitialPos = XMMatrixInverse(nullptr, initialPose);
    }
    ...
    for (int i = 0; i < clusterCount; i++)
    {
        ...
        FbxCluster* fbxCluster = fbxSkin->GetCluster(i);
        //このボーンに影響を受ける頂点の数 
        int cnotrolPointIndicesCount = fbxCluster->GetControlPointIndicesCount();
        //このボーンに影響を受ける頂点の配列
        int* controlPointIndices = fbxCluster->GetControlPointIndices();
        double* contorlPointWeights = fbxCluster->GetControlPointWeights();

        //影響を受ける全頂点について
        for (int j = 0; j < cnotrolPointIndicesCount; j++)
        {
            //頂点番号
            int vertIndex = controlPointIndices[j];
            //スキンウェイト
            float weight = (float)contorlPointWeights[j];
