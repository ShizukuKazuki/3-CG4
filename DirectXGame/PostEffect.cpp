...
//静的メンバ関数の実態
const float PostEffect::clearColor[4] = { 0.25f,0.5f,0.1f,0.0f }; //緑ぽい色
...
void PostEffect::Draw(ID3D12GraphicsCommandList* cmdList)
{
	...
	//(descHeap->GetGPUDescriptorHandleForHeapStart(), this->texNumber, descriptorHandleIncrementSize));
	...
	for (int i = 0; i < 2; i++)
	{
		...
		device->CreateRenderTargetView(texBuff[i].Get(),
			...
			CD3DX12_CPU_DESCRIPTOR_HANDLE(
				descHeapRTV->GetCPUDescriptorHandleForHeapStart(), i, device
...
void PostEffect::PreDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	...
	//深度ステンシルビュー用、、、、
	D3D12_CPU_DESCRIPTOR_HANDLE dsvH = descHeapDSV->
	...
	for (int i = 0; i < 2; i++)
	{
		...
		(0, 0, WinApp::window_width, WinApp::window_height);
	}
...
void PostEffect::CreateGraphicsPipelineState()
{
	...
	gpipeline.DepthStencilState.DepthFunc = D3D12_COMPARISON_FUNC_ALWAYS; // 常に
	...
	(&gpipeline, IID_PPV_ARGS(&pipelineState));
	assert(SUCCEEDED(result));
